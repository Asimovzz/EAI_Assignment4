...
class RobotWorkspace(BaseWorkspace):
    ...
    def _load_checkpoint_distributed(
        ...
        if self.is_main_process:
            payload_list[0] = torch.load(ckpt_path.open("rb"), pickle_module=dill, map_location="cpu")
        dist.broadcast_object_list(payload_list, src=0)
        payload = payload_list[0]
        self.load_payload_dict(
            payload,
            exclude_keys=exclude_keys,
            include_keys=include_keys,
        )
    ...
    def run(self):
        ...
        try:
            ...
            if cfg.training.resume:
                ...
                resume_training_state = bool(cfg.training.get("resume_training_state", True))
                if resume_optimizer and not resume_training_state:
                    raise ValueError(
                        "training.resume_optimizer=True requires "
                        "training.resume_training_state=True"
                    )

                exclude_keys = None if resume_optimizer else ("optimizer",)
                # An empty tuple intentionally restores no pickled workspace state,
                # leaving epoch/global_step at zero for weights-only finetuning.
                include_keys = None if resume_training_state else ()
                ...
                payload = self._load_checkpoint_distributed(
                    lastest_ckpt_path,
                    exclude_keys=exclude_keys,
                    include_keys=include_keys,
                )
                checkpoint_global_step = self.global_step
                serialized_global_step = payload.get("pickles", {}).get("global_step")
                if serialized_global_step is not None:
                    checkpoint_global_step = int(dill.loads(serialized_global_step))
                del payload
                if self.is_main_process:
                    mode = "full-state resume" if resume_training_state else "weights-only finetune"
                    print(
                        f"Resume mode: {mode}; optimizer={resume_optimizer}; "
            ...
            if cfg.training.use_ema:
                ...
                if cfg.training.resume and resume_training_state:
                    # EMAModel itself is not serialized. Reconstruct its warmup
                    # progress so a full resume does not restart EMA at decay=0.
                    accumulation = max(1, int(cfg.training.gradient_accumulate_every))
                    ema.optimization_step = checkpoint_global_step // accumulation
